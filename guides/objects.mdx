...
What should I consider when building my account and/or integrating with the API?
But first, what are placements?
We’ll cover placements in greater detail as we work through all of the previous questions. However, it’s important to understand what a placement is, and how it works, to have context when reading about all the other objects. 

Placement Creation Workflow


What’s happening?
When a placement object is created it can be provided an employee, employer, client, and job object ID. Only the employee ID is required.
When the placement object is created, it will return the required tasks that must be completed by the employee and employer representative(s) based on the active policies in the account. 
Policies define when a task should be returned when creating a placement. 
Tasks can range from government required documents (I-9, W4, etc) to organization-specific documents (Employee handbook, Code of Conduct, etc).
These tasks are assigned to an employee to complete. 
...
Employee Object in Onboarded Dashboard
...
Placements
All of these objects come together in the placement object. The placement object can be thought of as a time-in-place joining object which generates tasks based on the provided information. For example, if only the employer and employee object are provided when creating a placement, then the placement object will only return the required tasks that match policies relevant for that employer and employee (based on information like address, name, etc). 
...
Best Practices
When building with Onboarded, there are several recommended ways to use the above objects.
Full Orchestration (Preferred)
In this implementation path, you’re accessing the full value that Onboarded has to offer. Namely, you’re providing Onboarded with all of the company organization objects and using the placement object to generate tasks for employees. 

Company Organizational Entities:
Employers: Created via the API and populated from data from integrating system
Information Provided: Name, Address, EIN, DBA (if applicable), Phone, NAICS Code, and any Custom Attributes. 
Clients: Created via the API and populated with data from the integrating system
Information Provided: Name, Address
Jobs: Created via the API and populated with data from the integrating system
Information Provided: Name, Address
⚠️Important: The job address will be used to determine what state-specific forms are required in most cases. If a job has the same name, but is going to be in a different state, you’ll want to create a separate job for those locations. 

All of the above objects should be created as the information is available. During initialization of the account, you may want to add existing employers, clients, and jobs. However, you can also choose to create them in Onboarded either when they are created in the ATS or when a placement is being created for those objects for the first time.

As an example, you could create the client object in Onboarded when the client object in the integrating system is created. The benefits being that you can have that client ID from Onboarded ready to go for any future placements. Alternatively, you could create the client object when a customer goes to create their first placement. In which case, you’d send a request to Onboarded to create the client object, then use the returned ID immediately in the placement object and also associate it with the client object in the integrating system. 
